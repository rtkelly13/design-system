...
const SAMPLE = path.join(import.meta.dirname, 'templates/sample.tsx');
...
describe('renderReport', () => {
  ...
  it('puts the requested rung on the root element', async () => {
    ...
    expect(html).toContain('[data-theme="sketch"]');
  }, 30_000);
  ...
  it('resolves a scoped ThemeProvider inside a report', async () => {
    ...
    expect(html).toContain('<html lang="en" data-theme="sketch"');
    expect(body).toContain('data-theme="sketch"');
    expect(body).not.toContain('data-theme="midnight"');
  }, 30_000);

  /*
   * The design system self-hosts its faces, so a rendered report reaches no
   * font CDN at all. This asserts the property the deleted `--offline` flag
   * used to provide conditionally: every report is offline-capable, and the
   * type still resolves because the `--ds-font-*` tokens declare real stacks.
   */
  it('reaches no font CDN, and still resolves a typeface', async () => {
    const { html } = await render({ input: FIXTURE, output: out('fonts.html') });
