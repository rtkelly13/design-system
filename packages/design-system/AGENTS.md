...
- `pnpm contrast:report:html` (workspace root): The same audit as a report — `reports/contrast.tsx`.
...
What stays here is the **frame** — `ReportDocument`, `ReportSection` and
...
its baseline, and the generator asserts its pipeline against its own templates.
A story here that imported the generator's sample would invert the dependency.

`reports/contrast.tsx` is a report *about* this package, rendered with
`pnpm contrast:report:html` from the workspace root. It imports this package by
name, which resolves by Node self-reference through the `exports` map.
