...
export function GraphSlope({ title, fromLabel, toLabel, items: itemsProp, children, palette, corner, className }: GraphSlopeProps) {
  ...
  return (
    <Graph title={title} className={className} corner={corner}>
      <GraphBody className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-2" aria-label={`${title} comparison`}>
          {items.map((row, index) => {
