...
5. **Visual Regression Testing** — the standards, the determinism contract and the operating instructions live in [`docs/visual-regression.md`](./visual-regression.md); these are the rules:
   ...
   - Runs strictly on **Linux CI** to avoid OS font rendering diffs. Tolerance is `maxDiffPixels: 0` **and** `threshold: 0`, which together are genuinely zero allowance. Both are needed: `maxDiffPixels` bounds how many pixels may *count* as different, and `threshold` decides whether a pixel counts at all. Leaving `threshold` at its 0.2 default cost a missed regression — a brand accent moving `#ec4899` → `#f955a4` scores 0.0023 in the YIQ space pixelmatch uses, 87× below the bar, and passed all 40 cases. #125 has the measurement. It was `maxDiffPixelRatio: 0.05`, which let ~46,000 pixels of a 1280x720 shot differ freely and was only ever exercised against a placeholder image. Raise it only to the smallest number a real run demands, and say which story forced it.
